function [root varargout] = qdsparf(polysys, stol)
...
while ~h
    ...
    % first QR of M'
    [Qs, Rs,Ps,info] = spqr(Ms',struct('permutation','vector','tol',stol));    
    fprintf(' info->tol = %e\n',info.tol);
    % determine rank of M
    ...
    % second QR of Vs'
    [Qv Rv Pv,info] = spqr(Vs',struct('Q','discard','permutation','vector','tol',stol));
    fprintf(' info->tol = %e\n',info.tol);
...
%         ns = getCra(Asys,sum(c)-n);
        ...
        [Q R P info] = spqr(Vs(ns,:),struct('Q','discard','permutation','vector'));
        fprintf(' info->tol = %e\n---\n',info.tol);
